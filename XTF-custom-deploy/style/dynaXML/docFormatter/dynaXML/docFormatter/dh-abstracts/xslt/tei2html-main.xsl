...
-->
   ...
   <!-- jsInit not needed if use window.onload in .js file -->
    ...
    <xsl:template match="s|seg|resp|emph|label|foreign|term|hi|quote[(contains(@rendition,'#sq') or contains(@rendition,'#dq')) and not(@prev) and not(@next)]|q[(contains(@rendition,'#sq') or contains(@rendition,'#dq')) and not(@prev) and not(@next)]">
        <xsl:choose>
    ...
    <xsl:template match="address|dateline|text|castList|docImprint|docTitle|epigraph|signed|sound|titlePage|addrLine|quote[contains(@rendition,'#block')]|opener|closer|imprimatur|floatingText|milestone">
        <div>
            <xsl:call-template name="atts"/>
            <xsl:apply-templates/>
        </div>
