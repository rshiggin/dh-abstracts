...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="teiHeader">
    <xsl:apply-templates select="fileDesc/sourceDesc/biblStruct/monogr">
  ...
  <xsl:template match="sourceDesc/biblStruct/monogr">
    <xsl:param name="type"/>
    <xsl:apply-templates>
  ...
  <xsl:template name="dc_author">
    <xsl:element name="creator">
      <xsl:attribute name="content">volume</xsl:attribute>
      <xsl:choose>
        <xsl:when test="./name/@type='pseudonym'">
